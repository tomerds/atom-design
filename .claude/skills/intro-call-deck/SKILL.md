...
- Brand: single accent `#ff4227`, white bg, black text; Cal Sans titles, DM Sans body; headshots grayscale + square-rounded; no em dashes; no "modules" language (sell as one package). Always set `font-weight: 400` on Cal Sans heading elements — browsers default `<h1>`/`<h2>`/`<h3>` to bold, which synthetic-bolds Cal Sans.
- **Never** put a colored left rule (vertical accent bar, e.g. `border-left: 3px solid var(--accent)`) on a panel or text block — that left-bar callout reads as AI "admonition box" boilerplate and is off-brand. Soft panels/tinted fills are fine; the colored edge rule is the problem. Emphasize with the deck's own tools: an accent eyebrow/label, a single accent word or phrase inside plain body text, or a chip.
